...
export async function queryPaginated<T = any>(
    ...
    try {
        ...
        console.error(
            `Database paginated query error for table ${table}:`,
            error,
        );
        ...
        for (const operation of operations) {
            ...
            switch (operation.type) {
                case "insert":
                    ...
                    break;
                case "update":
                    result = await update(operation.table, operation.data, {
                        filters: operation.filters,
                    });
                    break;
                case "delete":
                    result = await remove(operation.table, {
                        filters: operation.filters,
                    });
                    break;
                default:
...
export const tables = {
    ...
    jobs: {
        ...
        async getByEmployer(employerId: string, options: QueryOptions = {}) {
            return query("jobs", {
                ...options,
                filters: { ...options.filters, employer_id: employerId },
            });
        },
