...
export function applySecurityHeaders(response: NextResponse, nonce?: string): NextResponse {
  ...
  if (nonce) {
    headers["Content-Security-Policy"] = headers["Content-Security-Policy"].replace(/{NONCE}/g, nonce);
  } else {
    // Remove nonce placeholders if no nonce provided
    headers["Content-Security-Policy"] = headers["Content-Security-Policy"].replace(/'nonce-{NONCE}'/g, '');
  }
