...
export class SecurityHardener {
  ...
  static validateEnvironment(): {
    ...
    for (const envVar of this.CRITICAL_ENV_VARS) {
      ...
      if (!value) {
        errors.push(`Critical environment variable ${envVar} is not set`);
      } else if (this.isDefaultValue(value)) {
        errors.push(`Critical environment variable ${envVar} is using a default/fallback value`);
      }
    }
    ...
    for (const envVar of this.WARNING_ENV_VARS) {
      ...
      if (!value) {
        ...
        warnings.push(`Environment variable ${envVar} is using a default/fallback value`);
      }
    }
   ...
   */
  static validateApiKey(key: string, type: 'stripe' | 'openai' | 'anthropic' | 'supabase'): boolean {
    const patterns = {
