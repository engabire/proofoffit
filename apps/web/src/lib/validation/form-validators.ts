...
export const validators = {
  required: (message: string = 'This field is required'): FieldValidator => (value) => {
  ...
  email: (message: string = 'Please enter a valid email address'): FieldValidator => (value) => {
  ...
  minLength: (min: number, message?: string): FieldValidator => (value) => {
    if (!value) return { isValid: true }; // Allow empty if not required
    if (value.length < min) {
      return { isValid: false, error: message || `Must be at least ${min} characters long` };
    }
    return { isValid: true };
  },
  ...
  maxLength: (max: number, message?: string): FieldValidator => (value) => {
    if (!value) return { isValid: true }; // Allow empty if not required
    if (value.length > max) {
      return { isValid: false, error: message || `Must be no more than ${max} characters long` };
    }
    return { isValid: true };
  },
  ...
  min: (min: number, message?: string): FieldValidator => (value) => {
    ...
    if (isNaN(num) || num < min) {
      return { isValid: false, error: message || `Must be at least ${min}` };
    }
    return { isValid: true };
  },
  ...
  max: (max: number, message?: string): FieldValidator => (value) => {
    ...
    if (isNaN(num) || num > max) {
      return { isValid: false, error: message || `Must be no more than ${max}` };
    }
    return { isValid: true };
  },
  ...
  phone: (message: string = 'Please enter a valid phone number'): FieldValidator => (value) => {
    if (!value) return { isValid: true }; // Allow empty if not required
  ...
  url: (message: string = 'Please enter a valid URL'): FieldValidator => (value) => {
    if (!value) return { isValid: true }; // Allow empty if not required
    try {
      new URL(value);
      return { isValid: true };
  ...
  password: (message?: string): FieldValidator => (value) => {
    ...
    if (feedback.length > 0) {
      return {
        isValid: false,
        error: message || `Password must contain ${feedback.join(', ')}`,
      };
    }
  ...
  confirmPassword: (passwordField: string, message?: string): FieldValidator => (value, formData) => {
    if (!value) return { isValid: true }; // Allow empty if not required
    if (value !== formData?.[passwordField]) {
      return { isValid: false, error: message || 'Passwords do not match' };
    }
    return { isValid: true };
  },
  ...
  oneOf: (options: any[], message?: string): FieldValidator => (value) => {
    ...
    if (!options.includes(value)) {
      return { isValid: false, error: message || `Must be one of: ${options.join(', ')}` };
    }
    return { isValid: true };
  },

  custom: (validator: (value: any, formData?: any) => ValidationResult): FieldValidator => (value, formData) => {
...
export const schemas = {
  ...
  signUp: {
    ...
    userType: [validators.required(), validators.oneOf(['seeker', 'employer'])],
  ...
  changePassword: {
    ...
    confirmPassword: [validators.required(), validators.confirmPassword('newPassword')],
  },
  ...
  candidateProfile: {
    ...
    phone: [validators.phone()],
    location: [validators.maxLength(100)],
    experience: [validators.required(), validators.oneOf(['entry', 'mid', 'senior', 'executive'])],
  ...
  employerProfile: {
    ...
    companySize: [validators.required(), validators.oneOf(['startup', 'small', 'medium', 'large', 'enterprise'])],
    location: [validators.maxLength(100)],
  ...
  jobPosting: {
    ...
    employmentType: [validators.required(), validators.oneOf(['full-time', 'part-time', 'contract', 'internship'])],
    experienceLevel: [validators.required(), validators.oneOf(['entry', 'mid', 'senior', 'executive'])],
  ...
  feedback: {
    ...
    category: [validators.required(), validators.oneOf(['bug', 'feature', 'improvement', 'other'])],
  },
};
