...
class PerformanceMonitor {
    ...
    if ('memory' in performance) {
      ...
      setInterval(() => {
        ...
        if (memoryGrowth > 10 * 1024 * 1024) { // 10MB growth
          this.metrics.memoryLeaks = (this.metrics.memoryLeaks || 0) + 1;
        }
  ...
  public generateRecommendations(): string[] {
    ...
    if (metrics.lcp && metrics.lcp > 2500) {
      recommendations.push('Optimize Largest Contentful Paint: Consider image optimization, preloading critical resources, or reducing server response time');
    }

    if (metrics.fid && metrics.fid > 100) {
      recommendations.push('Improve First Input Delay: Reduce JavaScript execution time, use code splitting, or defer non-critical scripts');
    }

    if (metrics.cls && metrics.cls > 0.1) {
      recommendations.push('Reduce Cumulative Layout Shift: Set size attributes on images, avoid inserting content above existing content');
    }

    if (metrics.pageLoadTime && metrics.pageLoadTime > 3000) {
      recommendations.push('Optimize page load time: Enable compression, minimize resources, use CDN, or implement caching');
    }

    if (metrics.memoryUsage && metrics.memoryUsage > 0.8) {
      recommendations.push('Optimize memory usage: Check for memory leaks, reduce bundle size, or implement lazy loading');
    }

    if (metrics.bundleSize && metrics.bundleSize > 2) {
      recommendations.push('Reduce bundle size: Implement code splitting, tree shaking, or remove unused dependencies');
    }
  ...
  public async sendReport(endpoint: string = '/api/performance'): Promise<void> {
    try {
...
export function usePerformanceMonitor() {
  ...
  return {
    ...
    sendReport: (endpoint?: string) => getPerformanceMonitor().sendReport(endpoint),
  };
}
