...
class IntelligentAnalysisTrigger implements SmartAnalysisTrigger {
  ...
  shouldAnalyze(item: any): boolean {
    ...
    const isRecent =
      Date.now() - new Date(item.last_seen_at).getTime() < recentThreshold;
    const isPriorityDomain = priorityDomains.some((domain) =>
      item.source_domain.includes(domain)
    );
  ...
  private async processQueue(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `🧠 Processing ${this.analysisQueue.length} items for AI analysis`,
      );
        ...
        await Promise.all(
          batch.map(async (item) => {
            try {
              ...
              // eslint-disable-next-line no-console
              console.log(
                `✅ Smart-analyzed: ${item.title.substring(0, 50)}...`,
              );
            } catch (error) {
              // eslint-disable-next-line no-console
