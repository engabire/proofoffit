...
export class USAJobsConnector implements JobFeedConnector {
  ...
  async fetchJobs(
    params: { keywords?: string; location?: string; limit?: number },
  ...
  normalizeJob(rawJob: any): any {
    return {
      ...
      pay: rawJob.PositionRemuneration
        ...
        : null,
      description: rawJob.QualificationSummary ||
        rawJob.UserArea?.Details?.JobSummary,
  ...
  private extractRequirements(
  ...
  ): { must_have: string[]; preferred: string[] } {
    ...
    sentences.forEach((sentence) => {
      ...
      if (commonRequirements.some((req) => lowerSentence.includes(req))) {
        if (
          lowerSentence.includes("preferred") ||
          lowerSentence.includes("desired")
        ) {
...
export class JobFeedManager {
  ...
  async saveJobsToDatabase(jobResults: JobFeedResult[]): Promise<void> {
    ...
    for (const job of allJobs) {
      try {
        const jobId = `${job.source}-${job.title}-${job.org}`.replace(
          /[^a-zA-Z0-9-]/g,
          "-",
        ).toLowerCase();
