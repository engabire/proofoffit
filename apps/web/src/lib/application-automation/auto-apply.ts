...
export interface JobApplication {
  ...
  jobId: string
  userId: string
  status: 'pending' | 'submitted' | 'reviewed' | 'interview' | 'rejected' | 'accepted'
  appliedAt: Date
  source: string
  applicationData: {
    resumeUrl?: string
...
export class ApplicationAutomation {
  ...
  async shouldAutoApply(job: any, config: AutoApplyConfig): Promise<boolean> {
    ...
    // Check salary range
    if (preferences.salaryMin && job.salary?.min && job.salary.min < preferences.salaryMin) {
      return false
    }
    if (preferences.salaryMax && job.salary?.max && job.salary.max > preferences.salaryMax) {
      return false
    }
    ...
    if (todayApplications && todayApplications.length >= preferences.maxApplicationsPerDay) {
      return false
    }

    if (weekApplications && weekApplications.length >= preferences.maxApplicationsPerWeek) {
      return false
    }
  ...
  async autoApplyToJob(job: any, config: AutoApplyConfig): Promise<JobApplication | null> {
    try {
      // Generate tailored resume and cover letter
      const tailoredDocuments = await this.generateTailoredDocuments(job, config)
      ...
      if (config.notificationSettings.email || config.notificationSettings.inApp) {
  ...
  // Generate tailored resume and cover letter
  private async generateTailoredDocuments(job: any, config: AutoApplyConfig): Promise<{
    resumeUrl: string
    coverLetterUrl: string
    customAnswers: Record<string, string>
    documents: string[]
    ...
    return {
      resumeUrl: `/api/documents/generate-resume?jobId=${job.id}&userId=${config.userId}`,
      coverLetterUrl: `/api/documents/generate-cover-letter?jobId=${job.id}&userId=${config.userId}`,
      customAnswers: {},
      documents: []
    }
  }
  ...
  private async sendApplicationNotification(application: JobApplication, config: AutoApplyConfig): Promise<void> {
    try {
      const notification = {
        user_id: config.userId,
      ...
      if (config.notificationSettings.email) {
        ...
        console.log('Sending email notification for application:', application.id)
      }
    } catch (error) {
