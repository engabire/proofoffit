...
class ConsentLedger {
  ...
  getEntries(filters?: {
    ...
    if (filters) {
      ...
      if (filters.packageId) {
        filteredEntries = filteredEntries.filter(entry => entry.packageId === filters.packageId);
      }
      if (filters.consentId) {
        filteredEntries = filteredEntries.filter(entry => entry.consentId === filters.consentId);
      }
      if (filters.action) {
  ...
  getStats(): ConsentLedgerStats {
    ...
    this.entries.forEach(entry => {
      ...
      entriesByPackage[entry.packageId] = (entriesByPackage[entry.packageId] || 0) + 1;
    });
   ...
   */
  getUserSuccessRate(userId: string): { submitted: number; failed: number; successRate: number } {
    const userEntries = this.getEntries({ userId });
    const submitted = userEntries.filter(entry => entry.action === 'submitted').length;
    const failed = userEntries.filter(entry => entry.action === 'failed').length;
