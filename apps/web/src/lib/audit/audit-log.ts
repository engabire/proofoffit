...
class AuditLogger {
    ...
    getStats(): AuditLogStats {
        ...
        this.logs.forEach((log) => {
            entriesByAction[log.action] = (entriesByAction[log.action] || 0) +
                1;
            entriesByResource[log.resource] =
                (entriesByResource[log.resource] || 0) + 1;
            if (log.userId) {
                entriesByUser[log.userId] = (entriesByUser[log.userId] || 0) +
                    1;
            }
        });
