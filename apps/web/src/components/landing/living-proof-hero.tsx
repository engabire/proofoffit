...
function LivingProofCanvas({ lane = "seeker" }: { lane?: Lane }) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute -top-40 -left-32 h-80 w-80 rounded-full bg-gradient-to-br ${
          theme.background[0]
        } blur-[100px]`}
      />
      <div
        className={`absolute -bottom-32 -right-40 h-72 w-72 rounded-full bg-gradient-to-tr ${
          theme.background[1]
        } blur-[120px]`}
      />
      ...
      >
        ...
        <g
          className={lane === "seeker"
            ? "text-sky-500/30 dark:text-sky-400/25"
            : "text-emerald-500/30 dark:text-emerald-400/25"}
          opacity="0.6"
        >
        ...
        <g
          className={lane === "seeker"
            ? "text-indigo-500/30 dark:text-indigo-400/25"
            : "text-teal-500/30 dark:text-teal-400/25"}
          opacity="0.6"
        >
        ...
        <g
          className={lane === "seeker"
            ? "text-purple-500/30 dark:text-purple-400/25"
            : "text-cyan-500/30 dark:text-cyan-400/25"}
          opacity="0.6"
        >
  ...
  return (
    <div className="mx-auto mb-10 max-w-7xl px-6">
      <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-white/70 bg-white/80 p-2 text-sm shadow-lg shadow-slate-200/30 backdrop-blur-md supports-[backdrop-filter]:bg-white/60 dark:border-white/10 dark:bg-slate-950/50">
        <a
          className="inline-flex items-center gap-2 rounded-xl px-3 py-1.5 text-slate-700 transition hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary dark:text-slate-200 dark:hover:bg-slate-900"
          href="#demo"
        >
        ...
        <a
          className="inline-flex items-center gap-2 rounded-xl px-3 py-1.5 text-slate-700 transition hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary dark:text-slate-200 dark:hover:bg-slate-900 font-medium"
          href="#webinar"
        >
  ...
  return (
    <motion.div
      ...
      className={`relative w-full max-w-md overflow-hidden ${
        theme.buttonStyle === "rounded-2xl"
          ? "rounded-[1.75rem]"
          : "rounded-[1.5rem]"
      } border ${theme.cardBorder} ${theme.cardBg} p-6 ${theme.shadowStyle} backdrop-blur-xl transition-transform`}
    >
      <div className="mb-4 flex items-center justify-between text-[11px] font-semibold uppercase tracking-[0.3em] text-slate-400">
        <span className="inline-flex items-center gap-2">
          Fit Report
          <span
            className={`rounded-full ${theme.accentLight} px-2 py-0.5 text-[10px] ${theme.secondaryText} dark:${theme.accentDark}/40`}
          >
            Live
          </span>
        </span>
        <a
          href="#audit"
          className="inline-flex items-center gap-1 text-slate-500 transition hover:text-slate-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary dark:text-slate-300 dark:hover:text-slate-100"
        >
          Audit trail
      ...
      <div className="mb-4 flex items-end justify-between">
        <div>
          ...
          <div className={`mt-1 text-4xl font-semibold ${theme.primaryText}`}>
            78
          </div>
        ...
        <div className="rounded-xl border border-white/80 bg-white/70 px-3 py-2 text-xs text-slate-500 shadow-sm dark:border-white/10 dark:bg-slate-900/60 dark:text-slate-300">
          <span className="block font-semibold text-slate-600 dark:text-slate-100">
            Proof signals
          </span>
      ...
      <div className="mb-5 h-1.5 w-full overflow-hidden rounded-full bg-slate-200/70 dark:bg-slate-800/70">
        <div
          className={`h-full w-[78%] rounded-full bg-gradient-to-r ${theme.primary}`}
        />
      </div>

      <ul className={`mb-6 space-y-2 text-sm ${theme.mutedText}`}>
        <li className="flex items-center gap-2">
          <span className={`size-1.5 rounded-full bg-${theme.accent}`} />Signals
          verified via source-control activity + compliance logs
        </li>
        <li className="flex items-center gap-2">
          <span
            className={`size-1.5 rounded-full bg-${
              lane === "seeker" ? "indigo-400" : "teal-400"
            }`}
          />Stretch surfaced: infrastructure-as-code (coachable)
        </li>
        <li className="flex items-center gap-2">
          <span
            className={`size-1.5 rounded-full bg-${
              lane === "seeker" ? "purple-400" : "cyan-400"
            }`}
      ...
      <div
        aria-hidden
        className="absolute -left-3 top-16 h-6 w-6 rounded-full border border-white/70 bg-white shadow-md shadow-slate-200/40 dark:border-slate-800 dark:bg-slate-900"
      />
      <div
        aria-hidden
        className="absolute -right-3 bottom-16 h-6 w-6 rounded-full border border-white/70 bg-white shadow-md shadow-slate-200/40 dark:border-slate-800 dark:bg-slate-900"
      />
    </motion.div>
  );
}
...
export default function LivingProofHero(
  { highlights = [], trusted = [], lane = "seeker" }: HeroProps,
) {
  const theme = laneThemes[lane];
    ...
    <section className="relative isolate overflow-hidden py-16 sm:py-24">
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 items-start gap-12 px-6 lg:grid-cols-12">
        <div className="relative z-10 space-y-8 lg:col-span-6">
          <div
            className={`inline-flex items-center gap-2 rounded-full ${theme.accentLight}/80 px-3 py-1 text-xs font-semibold uppercase tracking-[0.35em] ${theme.secondaryText} shadow-sm ${theme.shadowStyle} dark:${theme.accentDark}/40`}
          >
            Beauty • Utility • Proof
          </div>
          <div className="space-y-4">
            <h1
              className={`font-serif text-4xl tracking-tight ${theme.primaryText} sm:text-5xl`}
            >
              {lane === "seeker"
                ? "Land interviews with proof. Grow with confidence."
                : "Hire with receipts. Scale with precision."}
            </h1>
          ...
          <div className="flex flex-wrap items-center gap-4">
            <a
              ...
              className={`inline-flex items-center justify-center px-6 py-3 text-sm font-semibold text-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-${theme.accent} focus-visible:ring-offset-2 bg-gradient-to-r ${
                lane === "seeker"
                  ? "from-sky-600 to-indigo-600 hover:from-sky-700 hover:to-indigo-700"
            ...
            <span className={`text-sm ${theme.mutedText} ml-2`}>
              {lane === "seeker"
                ...
                : "No setup • 5 minutes to first slate"}
            </span>
          </div>
          ...
          {!!highlights.length && (
            <div className="grid gap-3 sm:grid-cols-3">
              {highlights.map((item) => (
                <Card
                  key={item.label}
                  className="border border-white/60 bg-white/85 shadow-sm shadow-slate-200/40 backdrop-blur dark:border-white/10 dark:bg-slate-950/60"
                >
        ...
        <div className="mx-auto mt-16 max-w-6xl px-6">
          <div className="rounded-[1.75rem] border border-white/70 bg-white/80 p-6 shadow-lg shadow-slate-200/30 backdrop-blur dark:border-white/10 dark:bg-slate-950/50">
            <p className="text-xs font-semibold uppercase tracking-[0.45em] text-slate-400">
              Stewarded with teams from
            </p>
            <div className="mt-5 grid gap-4 text-center text-sm font-medium text-slate-500 sm:grid-cols-5">
              {trusted.map((label) => (
                <span
                  key={label}
                  className="rounded-xl border border-white/60 bg-slate-50/80 px-3 py-2 shadow-sm shadow-slate-200/30 dark:border-white/10 dark:bg-slate-900/60 dark:text-slate-300"
                >
                  {label}
                </span>
              ))}
