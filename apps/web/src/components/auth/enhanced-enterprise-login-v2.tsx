...
export default function EnhancedEnterpriseLoginV2({
  ...
  return (
    <div
      className={cn(
        "w-full min-h-screen grid md:grid-cols-2 bg-white",
        className,
      )}
    >
      {/* Left: Brand/Value */}
      ...
      >
        <div>
          ...
          <p className="max-w-sm text-slate-600">
            Hire with confidence. Streamline reviews, collaborate with your
            team, and prove compliance — fast and secure.
          </p>
        </div>
        <ul className="space-y-3 text-sm text-slate-600">
          <li className="flex items-start gap-2">
            <ShieldCheck className="h-5 w-5 mt-0.5" /> SSO, MFA, Passkeys.
          </li>
          <li className="flex items-start gap-2">
          ...
          <li className="flex items-start gap-2">
            <Info className="h-5 w-5 mt-0.5" /> SOC 2 + GDPR aligned.
          </li>
        ...
        <Card className="w-full max-w-xl shadow-xl border-slate-200">
          <CardContent className="p-8">
                ...
                {isHirer && suggested && (
                  <p className="text-xs text-neutral-600">
                    ...
                    <span className="font-medium">
                      {suggested.hint ?? suggested.label}
                    </span>{" "}
                    ...
                    {suggested
                      ? (
                        <Button
                          ...
                          onClick={() => handleSso(suggested.key)}
                          disabled={status === "working"}
                        >
                      ...
                      : null}
                    {DEFAULT_PROVIDERS.filter((p) => p.key !== suggested?.key)
                      .map((p) => (
                        <Button
                          key={p.key}
                          type="button"
            ...
            <div className="text-[11px] leading-relaxed text-neutral-500">
              ...
              <a className="underline" href={legalLinks.terms}>Terms</a> and
              {" "}
              <a className="underline" href={legalLinks.privacy}>
                Privacy Policy
              </a>
              {legalLinks.eeoc
                ? (
                  <>
                    and our{" "}
                    <a className="underline" href={legalLinks.eeoc}>
                      EEOC commitment
                    </a>.
                  </>
                )
...
if (typeof window !== "undefined" && process.env.NODE_ENV !== "production") {
  (function runDevTests() {
    const domainCases: Array<
      { email: string; expect: Provider["key"] | null }
    > = [
    ...
    for (const t of domainCases) {
      ...
      if (res !== t.expect) {
        ...
        console.error("getSuggestedProvider test failed:", {
          test: t,
          got: res,
        });
      }
