...
export function BusinessIntelligence() {
  ...
  const getTrendColor = (trend: string, metric: BusinessMetric) => {
    if (metric.name === 'Cycle Time' || metric.name === 'Cost per Story Point' || metric.name === 'Time to Market') {
            ...
            <CardContent className="p-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className={`font-medium ${getTrendColor(metric.trend, metric)}`}>
                ...
                <div className="text-xs text-gray-500">
                  {metric.value >= metric.target ? '✅ Target achieved' : '⚠️ Below target'}
                </div>
              </div>
            </CardContent>
          </Card>
              ...
              <div key={team.team} className="space-y-3">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span>Velocity</span>
                    <span className={getPerformanceColor(team.velocity, 8)}>{team.velocity}</span>
                  </div>
                  <Progress value={(team.velocity / 10) * 100} className="h-1" />
                  
                  <div className="flex justify-between text-sm">
                    <span>Quality</span>
                    <span className={getPerformanceColor(team.quality, 95)}>{team.quality}%</span>
                  </div>
                  <Progress value={team.quality} className="h-1" />
                  
                  <div className="flex justify-between text-sm">
                    <span>Satisfaction</span>
                    <span className={getPerformanceColor(team.satisfaction, 8)}>{team.satisfaction}/10</span>
                  </div>
                  <Progress value={(team.satisfaction / 10) * 100} className="h-1" />
                  
                  <div className="flex justify-between text-sm">
                    <span>Efficiency</span>
                    <span className={getPerformanceColor(team.efficiency, 85)}>{team.efficiency}%</span>
                  </div>
              ...
              {costAnalysis.map((cost, index) => (
                <div key={cost.category} className="space-y-2">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-gray-600 w-12 text-right">{cost.percentage}%</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <CardContent>
          ...
          <div className="mt-6 p-4 bg-blue-50 rounded-lg">
            ...
            <ol className="space-y-2 text-sm text-blue-800">
              <li>1. <strong>Process Optimization:</strong> Implement cycle time reduction initiatives focusing on automation and parallel work streams</li>
              <li>2. <strong>Capacity Planning:</strong> Review team capacity and consider cross-training to improve velocity</li>
              <li>3. <strong>Quality Focus:</strong> Maintain high quality standards while optimizing for speed</li>
              <li>4. <strong>Cost Management:</strong> Monitor tools and training ROI, optimize infrastructure spending</li>
            </ol>
          </div>
        </CardContent>
      </Card>
