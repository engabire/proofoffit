...
import { Bell, AlertTriangle, CheckCircle, Clock, Users, TrendingUp, TrendingDown } from 'lucide-react'
...
export function SmartNotifications() {
  ...
  const [activeFilter, setActiveFilter] = useState<'all' | 'unread' | 'critical' | 'wip' | 'velocity' | 'blocked'>('all')
    ...
    if (settings.wipAlerts) {
      ...
      if (wipCount > wipLimit) {
        newNotifications.push({
          ...
          message: `Current WIP (${wipCount}) exceeds limit (${wipLimit}). Consider finishing items before starting new ones.`,
          timestamp: new Date(),
          priority: 'high',
          category: 'wip',
          actionable: true,
    ...
    if (settings.velocityAlerts) {
      ...
      if (velocityTrend === 'declining' && currentVelocity < targetVelocity) {
        newNotifications.push({
          ...
          message: `Team velocity (${currentVelocity}) is below target (${targetVelocity}) and declining. Review sprint planning.`,
          timestamp: new Date(),
          priority: 'medium',
          category: 'velocity',
          actionable: true,
    ...
    if (settings.cycleTimeAlerts) {
      ...
      if (avgCycleTime > targetCycleTime * 1.5) {
        newNotifications.push({
          ...
          message: `Average cycle time (${avgCycleTime} days) is significantly above target (${targetCycleTime} days).`,
          timestamp: new Date(),
          priority: 'high',
          category: 'cycle-time',
          actionable: true,
    ...
    if (settings.blockedItemAlerts) {
      ...
      if (blockedCount > 0) {
        newNotifications.push({
          ...
          message: `${blockedCount} items have been blocked for more than 24 hours. Immediate attention required.`,
          timestamp: new Date(),
          priority: 'critical',
          category: 'blocked',
          actionable: true,
    ...
    if (settings.deadlineAlerts) {
      ...
      if (daysToSprintEnd <= 2 && incompleteItems > 0) {
        newNotifications.push({
          ...
          message: `${daysToSprintEnd} days left in sprint with ${incompleteItems} incomplete items.`,
          timestamp: new Date(),
          priority: 'high',
          category: 'deadline',
          actionable: true,
    ...
    if (settings.teamAlerts) {
      ...
      if (teamLoad > 80) {
        newNotifications.push({
          ...
          message: `Team utilization is at ${teamLoad}%. Consider redistributing work or adjusting sprint scope.`,
          timestamp: new Date(),
          priority: 'medium',
          category: 'team',
          actionable: true,
