...
describe.skip("Tailor Engine", () => {
  ...
  const mockJob = {
    ...
    workType: "hybrid",
    description:
      "We are looking for a senior frontend developer with 5+ years of React experience and strong TypeScript skills.",
    requirements: {
    ...
    bullets: [
      {
        id: "bullet-1",
        text:
          "Led a team of 8 engineers to deliver a React-based healthcare application",
        tags: {
      ...
      },
      {
        id: "bullet-3",
        text:
          "Improved application performance by 40% using React optimization techniques",
        tags: {
  ...
  describe("tailorDocument", () => {
    ...
    it("handles job not found error", async () => {
      ...
      await expect(tailorEngine.tailorDocument(request)).rejects.toThrow(
        "Job not found",
      );
    });
    ...
    it("selects relevant bullets based on job requirements", async () => {
      ...
      const bulletTexts = result.citations.map((citation: { text: string }) =>
        citation.text
      );
      ...
      expect(bulletTexts).toContain(
        "Built advanced TypeScript components with 95% test coverage",
      );
    });
