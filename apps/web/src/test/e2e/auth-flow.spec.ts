...
test.describe('Authentication Flow', () => {
  ...
  test.describe('Sign Up Flow', () => {
    ...
    test('should allow audience selection', async ({ page }) => {
      ...
      await expect(page.locator('text=Job Seeker').locator('..')).toHaveClass(/data-state="active"/);
      
      // Click on Employer tab
      await page.click('text=Employer');
      
      // Check that Employer is now selected
      await expect(page.locator('text=Employer').locator('..')).toHaveClass(/data-state="active"/);
    });
  });
  ...
  test.describe('OAuth Authentication', () => {
    ...
    test('should initiate Microsoft OAuth flow', async ({ page }) => {
      ...
      // Should redirect to OAuth provider
      await expect(page).toHaveURL(/.*\/auth\/v1\/authorize.*provider=azure/);
    });
  });
