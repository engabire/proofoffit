...
describe.skip("Policy Engine", () => {
  ...
  describe("checkJobSourcePolicy", () => {
    ...
    it("returns deny for disallowed domains", async () => {
      ...
      const result = await policyEngine.checkJobSourcePolicy(
        "sketchy-jobs.com",
      );
  ...
  describe("validateJobCompliance", () => {
    it("validates compliant job posting", async () => {
      const job = {
        ...
        description:
          "We are looking for a skilled software engineer with 5+ years of experience in React and TypeScript. The role involves building modern web applications and working with cross-functional teams.",
      };
    ...
    it("detects non-compliant job posting", async () => {
      ...
      expect(result.violations).toContain(
        "Source sketchy-jobs.com is not allowed for automation",
      );
      expect(result.violations).toContain("Job title is too short or missing");
      expect(result.violations).toContain(
        "Job description is too short or missing",
      );
    ...
    it("detects federal job without salary information", async () => {
      const job = {
        ...
        title: "Software Engineer",
        description:
          "We are looking for a skilled software engineer with 5+ years of experience in React and TypeScript.",
      };
      ...
      expect(result.warnings).toContain(
        "Federal jobs should include salary information",
      );
    });
  });
  ...
  describe("updatePolicySource", () => {
    it("updates policy source successfully", async () => {
      mockQueryBuilder.update().eq.mockReturnValue({
        data: {},
        error: null,
      });
