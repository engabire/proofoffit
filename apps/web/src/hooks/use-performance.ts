...
export function usePerformance() {
  ...
  const collectMetrics = useCallback(() => {
    ...
    if (navigation) {
      ...
      newMetrics.timeToInteractive = navigation.domInteractive -
      ...
      list.getEntries().forEach((entry: any) => {
        switch (entry.entryType) {
          case "largest-contentful-paint":
            ...
            break;
          case "first-input":
            newMetrics.firstInputDelay = entry.processingStart -
              entry.startTime;
            break;
          case "layout-shift":
            if (!entry.hadRecentInput) {
              newMetrics.cumulativeLayoutShift =
                (newMetrics.cumulativeLayoutShift || 0) + entry.value;
            }
            break;
        }
      });
  ...
  const reportMetrics = useCallback(() => {
    ...
    const globalWindow = typeof window !== "undefined"
      ? (window as typeof window & { gtag?: (...args: any[]) => void })
      : undefined;
