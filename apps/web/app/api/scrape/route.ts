...
function assertAuthorized(req: NextRequest) {
  ...
  if (
    !process.env.SCRAPER_BEARER_TOKEN ||
    token !== process.env.SCRAPER_BEARER_TOKEN
  ) {
    throw new Error("401");
  }
...
async function conditionalGet(
  url: string,
  meta?: { etag?: string; lastmod?: string },
) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (process.env.NODE_ENV !== "production") {
        // eslint-disable-next-line no-console
        console.log(
          `Retry attempt ${attempt + 1} after ${delay}ms for: ${error.message}`,
        );
      }
      await new Promise((resolve) => setTimeout(resolve, delay));
    }
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.log("✅ Scraping job completed", {
      ...
      success_rate: results.filter((r) => r.status === 200).length /
        results.length,
    });
