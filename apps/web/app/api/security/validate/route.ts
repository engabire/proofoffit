...
function generateRecommendations(validation: {
  ...
  if (!validation.isValid) {
    recommendations.push('CRITICAL: Fix all security errors before production deployment');
  ...
  if (validation.warnings.length > 0) {
    recommendations.push('Review and configure optional environment variables');
    recommendations.push('Consider setting up monitoring for missing configuration');
  }
