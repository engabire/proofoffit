...
function assertAuthorized(req: NextRequest) {
  ...
  if (
    !process.env.AI_ANALYZER_TOKEN || token !== process.env.AI_ANALYZER_TOKEN
  ) {
    throw new Error("401");
  }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (process.env.NODE_ENV !== "production") {
      ...
      console.info("🧠 Starting AI content analysis", {
        mode,
        limit,
    ...
    if (process.env.NODE_ENV !== "production") {
      // eslint-disable-next-line no-console
      console.info(`Processing ${items.length} items for AI analysis`);
    }
        ...
        try {
          ...
          if (process.env.NODE_ENV !== "production") {
            ...
            console.info(
              ...
              }... (${analysis.sentiment}, priority: ${analysis.priority_score})`,
            );
          }
    ...
    try {
      ...
      if (process.env.NODE_ENV !== "production") {
        // eslint-disable-next-line no-console
        console.info(`🔍 Detected ${trends.length} trends`);
      }
    } catch (error) {
      if (process.env.NODE_ENV !== "production") {
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (type === "trends") {
      ...
      // Summary statistics
      const [trendsResult, analysisResult, categoriesResult] = await Promise
        .all([
          supabase.from("content_trends").select("trend_type", {
            count: "exact",
          }),
          supabase.from("content_analysis").select(
            "sentiment, primary_category",
            { count: "exact" },
          ),
          supabase.from("content_analysis").select("primary_category").not(
            "primary_category",
            "is",
            null,
          ),
      ...
      const sentimentStats =
        analysisResult.data?.reduce((acc: any, analysis) => {
          acc[analysis.sentiment] = (acc[analysis.sentiment] || 0) + 1;
          return acc;
        }, {}) || {};

      const categoryStats =
        categoriesResult.data?.reduce((acc: any, analysis) => {
          acc[analysis.primary_category] =
            (acc[analysis.primary_category] || 0) + 1;
          return acc;
        }, {}) || {};
